...
**Why this lab**: You've built each piece (session start, flows, domain) in isolation. Here you run one full conversation and watch which pattern and flows trigger at each step. That reinforces how the agent decides what to do and makes it easier to fix issues when a flow doesn't run or the wrong one runs.
