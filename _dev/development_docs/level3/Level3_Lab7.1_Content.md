...
b) The agent remembers the account number across turns in the same session  
c) The slot is never cleared  
d) The agent asks for the account every time  
...
|---|--------|-------------------|
| 1 | **b** | Slot persistence means the agent keeps the slot value across turns in the same conversation. |
| 2 | **c** | The Level 3 agent includes all Level 1, 2, and 3 flows (greet, hours, check_balance, etc.). |
