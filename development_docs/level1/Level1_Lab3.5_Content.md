...
**Why this lab**: In **Lab 3.4** you completed the hours and balance flow YAML with fill-in-the-blank exercises. Here you are given **exact, pre-written versions** of the responses and the corresponding flow files so your `level1` project matches what the **Lab 3.5** Check It! assessment expects.

**Before You Begin**: You’ve passed both fill-in-the-blank checks in **Lab 3.4** (or at least understand flow structure). You are in the `level1` project (folder that contains `domain/`, `data/`, `config.yml`).
