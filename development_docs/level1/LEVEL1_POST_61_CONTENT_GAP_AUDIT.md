...
- **Lab 7.1 grader**: `hours.yml` and `balance.yml` are in the known-files list so Lab 7.1 requires a new flow beyond those from Lab 3.5.
...
- **Gap?** **No.** Lab 6.1 text says "Before you begin: Finish Lab 3.5" (complete agent).
...
- **Gap?** **Yes (grading).** The Lab 7.2 grader treats as "known" only: greet, help, contact, goodbye, check_balance. It does **not** treat `hours.yml` or `balance.yml` as "already from the course." So a student who has only done through **Lab 3.4** (fill-in flows only; not the full **Lab 3.5** domain handout) can still have hours + balance files and may pass Lab 7.2 without a genuinely new feature—the grader can accept hours or balance as the "new" flow.
