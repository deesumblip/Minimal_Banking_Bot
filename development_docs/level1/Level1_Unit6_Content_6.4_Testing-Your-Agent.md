...
**Why test deliberately**: Clicking around once isn't enough. You want to verify each flow (greet, help, contact, goodbye, hours, and balance) with different phrasings so you see that the LLM matches correctly and the right response appears. That catches bad descriptions or missing responses before anyone else uses the agent.
