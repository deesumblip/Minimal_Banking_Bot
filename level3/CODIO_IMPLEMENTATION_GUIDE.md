...
- **Graded labs**: Lab 3.1, 4.1 (LLM Rubric with solution references in `.guides/assessments/level3_graders/`), Lab 6.1 (Python grader `lab_6.1_grader.py`). Working Directory for assessments: `/home/codio/workspace`.
...
- **Lab 3.1 / 4.1**: Use LLM Rubric Autograde with the instructor-provided solution files in `.guides/assessments/level3_graders/` (see each lab’s Assessment_Setup.md). Option B: Standard Code Test with Bash scripts if you add `lab_3.1_grader.sh` / `lab_4.1_grader.sh`.
