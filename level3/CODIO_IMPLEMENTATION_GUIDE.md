...
- **Guide page JSON (Chapter 1.3)**: All student pages use `layout: "2-panels-tree-guides-left"` and `path: ["level3"]` so the file tree stays open and focused on `level3`; clicking a file opens it in the secondary panel (right). **`files[0].path`**: use `""` for read-only / concept pages (Units 0–3.1 content, 4.x–5.x content, 6.3–6.4, 7.2–7.3, 8.x); use `"#terminal"` for any page where students run the terminal (`venv`, `cd level3`, `rasa train`, `rasa inspect`) — **all labs (3.1–7.1)** plus content pages **6.1 Training with Slots**, **6.2 Testing Slot Collection**, and **7.1 Complete Bot Walkthrough** (same pattern as Chapter 1.2’s `level2` pages).
