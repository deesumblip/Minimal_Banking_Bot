...
**2. When a flow has a `collect: account` step and the `account` slot is empty, what does the agent do?**
...
| 2 | **b** | The `collect:` step gets the slot value; if empty, the agent asks using `utter_ask_*` and stores the answer. |
