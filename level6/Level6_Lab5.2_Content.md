...
**Step 2.** Run the assessment. The grader will check for: level6/sub_agents/banking_assistant/config.yml, level6/endpoints.yml with mcp_servers, level6/data/basics/ask_banking_assistant.yml with a flow that has `call: banking_assistant`, and optionally level6/models/ or successful train.
...
From project root activate the venv, `cd level6`. Run the assessment when ready.
