...
- Create a **sub-agent config** in `sub_agents/<name>/config.yml` (agent name, protocol, description, LLM, mcp_servers).
- Add **mcp_servers** to **endpoints.yml** (name, url, type) so the sub-agent can use MCP tools.
- Create a **flow** that includes a step **call: banking_assistant** (and optionally a follow-up action).
- **Train** the Level 6 bot from the level6 folder and run the **completion check** (Lab 5.2).
