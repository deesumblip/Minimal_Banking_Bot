...
### Grading Tab
- **Points**: `12`
- **Allow partial points**: `OFF` (single test case; script reports one score)
- **Use maximum score**: `OFF`
- **Case insensitive**: `ON`
- **Ignore white spaces**: `ON`
- **Substring match**: `ON` (output must *contain* the expected string)
- **Test case** (single case):
  - **INPUT – Arguments**: leave empty
  - **INPUT – STDIN**: leave empty
  - **Expected output**: ` PASS: Action creation verification complete!` (include the leading space)
  - **Show error feedback**: optional; `OFF` is fine
- **Show expected answer**: `ALWAYS` (so students see the required phrase)
- **Show rationale to student**: `NEVER` (or set as desired)
- **Defined number of attempts**: `OFF` (or set if you want a limit)
...
6. In the **Grading** tab:
   - Set Points: `12`
   - Set **Substring match** to `ON`, **Case insensitive** and **Ignore white spaces** to `ON`
   - Add one test case: leave **INPUT – Arguments** and **INPUT – STDIN** empty; set **Expected output** to ` PASS: Action creation verification complete!` (with the leading space)
   - Optionally set **Show expected answer** to "ALWAYS" and **Show rationale** as desired
7. In the **Files** tab:
