...
After you run the Lab 4.1 assessment, you can see how registration affects the bot. **In the terminal**, navigate to the **`level2`** folder (the one that contains `config.yml`, `domain/`, `data/`, and `actions/`). For example: `cd level2` if you're in the project root, or use the full path to your `level2` directory. With the virtual environment active, run `python -m rasa train`. Then start and open the Rasa Inspector GUI (see Unit 6.3 for how). In the Inspector chat, try:
