...
This document is **not** part of the student-facing Codio guide. It explains how to configure Codio so the terminal can auto-start Inspector when a student opens **6.3 Testing Your Agent** (Level 1, Unit 6), and how to use the **Start Rasa Inspector** link if auto-run is unavailable.
...
1. Open **Level 1** → **Unit 6** → **6.3 Testing Your Agent** (the command belongs on the **6.3** page layout in the Guide Editor, not on a separate student page).
