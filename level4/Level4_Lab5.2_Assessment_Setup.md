...
**Task.** Students run **Check It!** (completion check) first; it verifies domain, action file, flow file, and model—**without** starting Rasa. **Optional:** Start the agent (`rasa inspect` / `rasa run`), open **Rasa Inspect**, walk through the transfer flow, and confirm `action_process_transfer`. Guide page order: Part 1 = assessment, Part 2–3 = Inspector (optional).
