...
1. **Pipeline (Lab 0.1)** — Built **`level4/config.yml`** and **`level4/endpoints.yml`** (fill-in-the-blanks, paste, code test) so Level 4 uses **Compact** command generation and the course **`model_groups`** pattern for reliable multi-slot FillSlot.
