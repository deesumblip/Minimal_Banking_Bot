...
b) Several memory variables (e.g. amount, recipient, account_from) that the agent can collect and use together in one flow  
...
b) The order is the order in which the agent will ask the user for each value  
...
|----------|--------|-------------------|
| 1 | **b** | Multiple slots are several memory variables the agent collects in one conversation and can use together (e.g. in one action). |
| 2 | **b** | The order of `collect:` steps is the order in which the agent prompts for each slot value. |
