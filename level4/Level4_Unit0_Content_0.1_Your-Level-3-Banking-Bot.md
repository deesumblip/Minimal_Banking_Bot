...
When you move to Chapter 1.4, you work in **`level4/`**. The Level 4 assistant is **not** identical to Level 3 at the pipeline layer: you need **`CompactLLMCommandGenerator`**, **`flow_retrieval`**, and (in this course) an **`endpoints.yml`** **`model_groups`** profile suited to multi-slot FillSlot—see **Unit 0.3 Complete Delta** and **`level4/PIPELINE_CHAPTER_1_3_AND_4.md`**. Do **not** assume only **`assistant_id: level4-bot`** changes.
