This page lists **every** change from the **Chapter 1.3 end-state bot** to the **Chapter 1.4 end-state bot**. Use it as a checklist when you work in your own repo or when you compare **`level3/`** and **`level4/`**.

**Chapter 1.3 end state (what you bring forward)** — Typically includes:
...
You do **not** delete that work. Chapter 1.4 adds the **transfer** pieces and adjusts **pipeline** files so multi-slot + free-text collection works reliably in **`level4/`**.
...
- If you copy **`level3/` → `level4/`** yourself, preserve **all** Level 3 files, then apply **sections 2–4** below.

---

## 2. Pipeline files (required — not optional for reliable CALM)
...
## 3. Lab deliverables (what you add in guided labs)
...
| **Lab 5.2** | Completion check + Inspector (**scripted transfer** in the Lab 5.2 page) | Grader + Rasa Inspect |

---

## 4. What stays from Chapter 1.3

- All **existing** responses, flows, and actions that you already had (greet, help, hours, **check_balance**, etc.) remain in **`level4/`** unless you choose to refactor.
- **Optional:** If your Chapter 1.3 bot included **extra** flows (e.g. holiday hours), keep them when you copy to **`level4/`**. The course **`level4`** snapshot may not include every optional Level 3 file; your full bot can.
