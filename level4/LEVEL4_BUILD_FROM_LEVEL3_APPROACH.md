...
How **Level 4** relates to **Level 3** when you set up or teach the course.

---

## Goal

Students **start from Level 3 completion** ( **`level3/`** is the reference agent; **`level4/`** is the Level 4 workspace, already at that baseline in this repo) and **add** Level 4 content (transfer slots, `action_process_transfer`, `transfer_money.yml`) in guided labs, then align **`level4/`** pipeline files, train, and test.

**Level 4 instructional markdown** assumes that baseline everywhere (Unit 0, labs, guides).
...
## 2. Pipeline files (Level 4)

See **Unit 0.2** (section 2) in the Level 4 guide. **`level4/config.yml`** uses **`CompactLLMCommandGenerator`**, **`flow_retrieval`**, etc.; **`level4/endpoints.yml`** aligns **`model_groups`** / **`temperature`** for FillSlot reliability.
...
| Topic | Detail |
|-------|--------|
| **Student story** | Level 4 **starts** at Level 3 **completion**; labs **add** transfer + pipeline alignment + train/test. |
