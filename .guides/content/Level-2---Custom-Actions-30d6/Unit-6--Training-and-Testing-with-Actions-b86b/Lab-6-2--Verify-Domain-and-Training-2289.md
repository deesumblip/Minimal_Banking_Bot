...
3. **Go into `level2`:** run `cd level2`. All commands below run from here so Rasa uses the Level 2 agent (your domain, flows, and actions).
4. **Train the Level 2 agent:** run `python -m rasa train`. Wait for the model to save.
...
Submit the Check It! below once you meet the checks (domain registration and a trained model).
