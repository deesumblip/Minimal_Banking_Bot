...
If training fails, check: (1) **`domain/basics.yml`** lists **`action_process_transfer_with_tools`** under **`actions:`** and **`amount`**, **`recipient`**, and **`account_from`** each include **`active_flow: transfer_money_tools`** under **`from_llm`** (as in **Lab 4.1**); (2) **`endpoints.yml`** has **`tools:`** with **`tools_module: "tools"`**; (3) **`tools/banking_tools.py`** exists with valid Python and **`__all__`** defined; (4) **`config.yml`** still points to your **`prompt_template`** (baseline **`command_prompt_v3_slot_names.jinja2`** in this repo—do not delete it accidentally).
