...
## Where Chapter 1.6 happens

Do all Chapter 1.6 work under **`level6/`**. Keep **`level5/`** as your reference for “what Chapter 1.5 completion looks like”; **`level6/`** extends that design with sub-agents, MCP registration for the specialist, and a new flow that **`call:`**s the sub-agent.

---

## What you have at Chapter 1.5 completion

### Domain (`level5/domain/basics.yml`)

- Responses: `utter_greet`, `utter_help`, `utter_contact`, `utter_goodbye`, and utterances for slots (`utter_ask_*`) as in Chapter 1.5.
...
- At minimum: `greet`, `help`, `contact`, `goodbye`, `hours`, `check_balance`, `transfer_money`, **`transfer_money_tools`** (as completed in Chapter 1.5).

### Tools (`level5/tools/`)

- **`banking_tools.py`** (or equivalent) with **`check_balance`**, **`process_transfer`**, **`get_account_info`** registered via **`endpoints.yml`** **`tools:`**.

### Config (Chapter 1.5 **done**)
...
## How Chapter 1.6 extends Chapter 1.5

Chapter 1.5 **does not** define sub-agents or MCP for a specialist. Chapter 1.6 **adds**:
