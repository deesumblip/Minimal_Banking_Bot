# Level 1 guide duplication: `Level-1-1-d3b4` vs `Level-1---Just-Responses-d3b4` (historical)

**Status: Resolved.** Level-1-1-d3b4 has been moved to **level1/old_duplicates/Level-1-1-d3b4**. The course now uses **Level-1---Just-Responses-d3b4** as the single source of truth. See **level1/MIGRATION_LEVEL1_CHAPTER.md** for how to populate that chapter in the repo from Codio.
...
| **Level-1---Just-Responses-d3b4** | No | Not in the repo. On Codio it has Units 0–6. |

The root `.guides/content/index.json` used to list only **Level-1-1-d3b4** as the first guide. So when you import from Git, Codio gets that truncated tree (0–3) and does not get **Level-1---Just-Responses-d3b4** from the repo. If you see **Level-1---Just-Responses-d3b4** on Codio, it was created there (or by a different import) and is the one with full Units 0–6.

So on Codio you effectively have two Level 1 chapters:

- **Level-1---Just-Responses-d3b4** – full 0–6 (only on Codio).
- **Level-1-1-d3b4** – 0–3 (from repo). Units 0–3 overlap with the chapter above → **duplication**.

## How to check if Level-1-1-d3b4 overlaps Level-1---Just-Responses-d3b4 (Units 0–3)
...
- **Level-1---Just-Responses-d3b4** is only on Codio. To get it:
  ...
  - Copy the folder from the Codio workspace (e.g. `guides/content/Level-1---Just-Responses-d3b4`) into this repo, e.g. under `.guides/content/` for the comparison.

Put the copied chapter next to Level-1-1, e.g.:

- `.guides/content/Level-1-1-d3b4/`
- `.guides/content/Level-1---Just-Responses-d3b4/`   (temporary copy)
...
$C = ".guides/content/Level-1---Just-Responses-d3b4"
...
diff (Get-Content ".guides/content/Level-1-1-d3b4/$f") (Get-Content ".guides/content/Level-1---Just-Responses-d3b4/$f")
```
...
- **Keep** the chapter that has the full course (Units 0–6): **Level-1---Just-Responses-d3b4** on Codio.
- **Remove the duplicate**:
  ...
  - On **Codio**: remove or hide the **Level-1-1-d3b4** guide so only **Level-1---Just-Responses-d3b4** appears.

Then either:

- **Option A – Codio as source of truth for Level 1:** Keep **Level-1---Just-Responses-d3b4** only on Codio; repo no longer has a Level 1 guide (or has a copy for reference only). Root `index.json` would list only **Level-2---…** onward, or you add **Level-1---Just-Responses-d3b4** to the repo by copying it from Codio and add it to `index.json`.  
- **Option B – Repo as source of truth:** Copy the full **Level-1---Just-Responses-d3b4** (0–6) from Codio into the repo as the single Level 1 chapter, update root `index.json` to point to it, then remove **Level-1-1-d3b4** so there is no duplication.
