**Starting point:** Before **Lab 2.1**, align **`level4/config.yml`** and **`level4/endpoints.yml`** with **Unit 0.2** (section 2) so CALM can fill free-text slots reliably. The baseline agent you extend is summarized in **Unit 0.1**. All work in this lab is under **`level4/`**.

**Repository note:** The checked-in **`level4/`** starter may still use the **SearchReady** pipeline until you replace it here. Use the fill-in-the-blanks, then paste into **`level4/config.yml`** and **`level4/endpoints.yml`** (fill-in-the-blanks → paste → code test).
...
The exercise has **Part A** (`config.yml`) and **Part B** (`endpoints.yml`). It matches **Unit 0.2** (pipeline and endpoints).
...
1. Open **`level4/config.yml`** in the editor (or create the file if it is missing).
