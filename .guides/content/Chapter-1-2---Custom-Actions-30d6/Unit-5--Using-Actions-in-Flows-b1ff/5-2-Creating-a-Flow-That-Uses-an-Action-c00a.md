...
After completing Lab 5.1, train and open the Rasa Inspector GUI (Unit 6.3). In the chat, try these to verify your implementations:

- **"What are your hours?"** — Should trigger `hours` flow and `action_bank_hours`; message varies by day.
- **"What are your holiday hours?"** or **"Are you open on Christmas?"** — Should trigger `holiday_hours` flow and `action_holiday_hours`; message varies by whether today is a holiday.
- **"Hello"** — Should trigger `greet` flow (Level 1).

If something doesn’t work, re-train after changing flows, check the flow’s `description` in the YAML, and see Unit 6.4 for debugging.
