...
1. **Start the assistant** from **`level4`** (same shell as Lab 5.1 after `cd level4`), e.g. `python -m rasa inspect --debug --log-file logs/logs.out`. That matches the project’s **`level4/README.md`** *—repository Markdown; not a guide page* and writes logs under **`level4/logs/`** for debugging. If your setup prefers chat-only, `python -m rasa run` from **`level4`** is fine—always use the venv’s **`python -m rasa …`**, not a global `rasa` binary, so you pick up the right install. Leave the process running.
