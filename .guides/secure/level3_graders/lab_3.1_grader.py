...
Checks level3/domain/basics.yml for: file exists, valid YAML, slots: section,
account slot (text), utter_ask_account response, and the three expected actions.
...
if not DOMAIN_PATH.exists():
    ...
    print("FAIL")
    sys.exit(1)
print("✅ Check 0: PASSED - domain file exists (1 point)")
score += 1
print("")

# Check 1: Valid YAML (gate; must pass to continue; no points)
print("Check 1: Verifying YAML syntax and top-level structure...")
data = None
yaml_err = ""
try:
...
if yaml_err:
    print("❌ Check 1: FAILED - domain/basics.yml has YAML syntax errors (0 points)")
    print(f"   {yaml_err}")
    print("Hint: Check indentation (2 spaces), colons, and list dashes. Avoid tabs.")
    print("FAIL")
    sys.exit(1)
if not isinstance(data, dict):
    print("❌ Check 1: FAILED - domain file must be a YAML mapping (key-value root) (0 points)")
    print("Hint: The file should start with keys like version:, responses:, actions:, not a bare list.")
    print("FAIL")
    sys.exit(1)
print("✅ Check 1: PASSED - valid YAML with a top-level mapping (required for following checks)")
print("")
...
# Check 2: slots section (2 points)
print("Check 2: Verifying slots: section...")
if not isinstance(slots, dict):
    print("❌ Check 2: FAILED - No top-level 'slots:' section or it is not a mapping (0 points)")
...
# Check 3: account slot with type text (2 points; 1 partial)
print("Check 3: Verifying account slot (type text)...")
if not isinstance(slots, dict):
    print("❌ Check 3: FAILED - slots section missing (0 points)")
    ...
    if slot_type == "text" or (
        ...
        print("⚠️ Check 3: PARTIAL - account slot exists but type should be 'text' (1 point)")
        score += 1
    else:
        print("❌ Check 3: FAILED - account slot must be a mapping with type: text (0 points)")
print("")

# Check 4: utter_ask_account in responses (2 points; 1 partial)
print("Check 4: Verifying utter_ask_account response...")
if not isinstance(responses, dict):
    print("❌ Check 4: FAILED - No 'responses:' section or it is not a mapping (0 points)")
    ...
    if has_text:
        ...
        print("⚠️ Check 4: PARTIAL - utter_ask_account present but needs at least one text (1 point)")
        score += 1
print("")

# Check 5: action_check_balance_simple in actions (1 point)
print("Check 5: Verifying action_check_balance_simple in actions...")
if not isinstance(actions, list):
    print("❌ Check 5: FAILED - No 'actions:' list or it is not a list (0 points)")
    ...
    print("❌ Check 5: FAILED - action_check_balance_simple not in actions list (0 points)")
    print("Hint: Add '- action_check_balance_simple' under the actions: section")
else:
    print("✅ Check 5: PASSED - action_check_balance_simple registered (1 point)")
    score += 1
print("")

# Check 6: action_bank_hours (1 point)
print("Check 6: Verifying action_bank_hours in actions...")
if not isinstance(actions, list):
    print("❌ Check 6: FAILED - actions list missing (0 points)")
elif "action_bank_hours" not in actions:
    print("❌ Check 6: FAILED - action_bank_hours missing from actions (0 points)")
    print("Hint: Include '- action_bank_hours' in the actions: list (see Lab 3.1 example).")
else:
    print("✅ Check 6: PASSED - action_bank_hours present (1 point)")
    score += 1
print("")

# Check 7: action_holiday_hours (1 point)
print("Check 7: Verifying action_holiday_hours in actions...")
if not isinstance(actions, list):
    print("❌ Check 7: FAILED - actions list missing (0 points)")
elif "action_holiday_hours" not in actions:
    print("❌ Check 7: FAILED - action_holiday_hours missing from actions (0 points)")
    print("Hint: Include '- action_holiday_hours' in the actions: list (pairs with holiday_hours flow).")
else:
    print("✅ Check 7: PASSED - action_holiday_hours present (1 point)")
    score += 1
print("")
