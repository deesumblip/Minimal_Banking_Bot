...
This document maps **Level 5** (Level 5 – Tool Calling) content files, labs, and assessment task IDs so implementers can sync with Codio and keep graders in one place.
...
|---------|---------------------|
| **Starter** = Level 3 end state (`level4/`) | **Starter** = Level 4 completion / final Level 4 bot (`level5/` as default baseline in repo) |
...
**Lab numbering:** Level 5 uses **Lab 2.0** (command-generator prompt; **code test** 10 points), then Labs 2.1, 3.1, 4.1, 5.1, 5.2. **Lab 2.0** wiring is also rechecked in **5.1 / 5.2** graders. **Lab 2.1** uses a **fill-in-the-blanks** exercise (5 points) plus the **code test** (10 points), matching the Level 4 Lab 0.1 pattern. Labs 3.1–5.2 use code tests only (10 points each). **Total graded points:** 65.
...
| File | Purpose |
|------|---------|
| `Level5_Unit0_Content_0.1_Your-Level-4-Banking-Agent.md` | Recap: Level 4 completion → `level5/` baseline; domain, flows (incl. goodbye, holiday_hours), actions. |
...
|-----|--------------|------------------------|-----------------|------------------------|
| 2.0 | Codio: `Lab-2-0-Command-generator-prompt-L5c0.md`; canonical template **`level5/resources/command_prompt_v3_slot_names.jinja2`** | `code-output-compare-501020000.json` | **code-output-compare-501020000** | `lab_2.0_grader.py`, `lab_2.0_solution_reference.md`; also **501050001** / **501050002** Check 3 |
...
| Graders | .guides/secure/level5_graders/lab_2.0_grader.py … lab_5.2_grader.py |
