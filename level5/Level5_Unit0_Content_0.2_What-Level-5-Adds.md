...
- Your **starting point** is **Chapter 1.4 completion** in **`level5/`**. The repo matches that baseline (no **`tools/`**, no **`tools:`** in **`endpoints.yml`**, no **`transfer_money_tools`** flow or **`action_process_transfer_with_tools`** yet, and no **`prompt_template`** / **`data/prompts/`** until **Lab 2.0**—the template to copy is **`resources/command_prompt_v3_slot_names.jinja2`**). You then add the **`tools/`** module, **`tools:`** in **`endpoints.yml`**, the **`transfer_money_tools`** flow, and **`action_process_transfer_with_tools`**, then train and test. Complete every lab so *you* produce those files; when you are done, your tree should match the **Chapter 1.5 end state** summarized again at the **end of this chapter**.
